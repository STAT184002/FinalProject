...
Happiness_Literacy %>%
  ...
  labs(x = "Happiness Score", y = "Literacy Rate", title = "Does Happiness Mean Literacy?")
```
...
Additioanlly when evaluating the rank variable, we can see that as the happiness score and literacy rate increases, the rank decreases showing an inverse relationship between the happiness score and literacy rate and the variable rank. This is a good thing because haivng a lower rank means that the country has a better score, literacy rate, GDP, social support, life expectancy, freedom, and generosity rates as well. 